...
def get_user_with_groups(
    ...
    user = objects[0][0]
    user_brood = objects[0][1]
    user_web3 = objects[0][2]
    groups = []
    ...
    return data.UserWithGroupsResponse(
        ...
        email=user_brood.email if user_brood is not None else None,
        normalized_email=user_brood.normalized_email
        if user_brood is not None
        else None,
        verified=user.verified,
        ...
        web3_address=user_web3.web3_address if user_web3 is not None else None,
        groups=groups,
    )
...
def get_current_user_by_token(
    ...
    """
    query = (
        session.query(Token.active, User)
...
def get_current_user_with_groups_by_token(
    ...
    return active_token, data.UserWithGroupsResponse(
        user_id=user.id,
        username=user.username,
        first_name=user.first_name,
        last_name=user.last_name,
