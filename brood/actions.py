...
class TokenNotActive(Exception):
    """
    Raised when token with the given parameters is not active.
    """
...
def get_user_query(
    ...
    """
    Apply filters to user query depends on argument provided.
    """
    if username is None and email is None and user_id is None and web3_address is None:
        raise UserInvalidParameters(
...
def get_user_with_groups(
    ...
    query = session.query(User, Group, GroupUser)
    query = get_user_query(
        query=query,
        username=username,
        email=email,
    ...
    query = (
        query.join(GroupUser, GroupUser.user_id == User.id, isouter=True)
        .join(Group, Group.id == GroupUser.group_id, isouter=True)
        .group_by(User, Group, GroupUser)
    )
    ...
    for object in objects:
        ...
        if group_user.user_id != user.id:
            logger.error(
                f"Unexpected group id: {group.id} fetched for user with id: {user.id}"
            )
            raise Exception("Unexpected group for user")
        groups.append(
    ...
    return data.UserWithGroupsResponse(
        user_id=user.id,
        username=user.username,
...
def get_current_user_by_token(
    ...
    """
    query = (
        session.query(Token.active, Token.restricted, User)
    ...
    is_token_restricted = objects[0][1]
    user = objects[0][2]

    return is_token_active, is_token_restricted, user


def get_current_user_with_groups_by_token(
    session: Session, token: uuid.UUID
) -> Tuple[bool, bool, data.UserWithGroupsResponse]:
    """
    Extend user data with groups for authentication middleware.
    """
    query = (
        session.query(Token.active, Token.restricted, User, Group, GroupUser)
        .join(User, User.id == Token.user_id)
        .join(GroupUser, GroupUser.user_id == User.id, isouter=True)
        .join(Group, Group.id == GroupUser.group_id, isouter=True)
        .filter(Token.id == token)
        .group_by(Token.active, Token.restricted, User, Group, GroupUser)
    )
    objects = query.all()
    if len(objects) == 0:
        raise TokenNotFound("Token not found")
    ...
    groups = []
    for object in objects:
        if object[2].id != user.id:
            logger.error(f"Found several users with token id: {token}")
            raise Exception("Found several users")
        # Skip if there are no groups
        if object[3] is None:
            continue
        if object[4].user_id != user.id:
            logger.error(
                f"Unexpected group id: {object[3].id} fetched for user with id: {user.id}"
            )
            raise Exception("Unexpected group in list")
        groups.append(
            data.GroupUserResponse(
                group_id=object[3].id,
                user_id=object[4].user_id,
                user_type=object[4].user_type,
