...
async def create_user_handler(
    ...
    try:
        ...
        raise HTTPException(
            status_code=422, detail="Username must not contain spaces",
        )
    except actions.PasswordInvalidParameters as invalid_password_error:
        raise HTTPException(
            status_code=422, detail=invalid_password_error.generic_error_message,
        )
    ...
    if SEND_EMAIL_WELCOME:
        background_tasks.add_task(actions.send_welcome_email, user=user)
...
async def restore_password_handler(
    ...
    try:
        ...
        background_tasks.add_task(
            actions.send_reset_password_email, reset_object=reset_object, email=email,
        )
        reset_status = "ok"
    except actions.UserNotFound:
...
async def delete_user_handler(
    ...
    try:
        user = actions.delete_user(
            session=db_session, user_id=user_id, current_password=password,
        )
    except actions.UserInvalidParameters:
        raise HTTPException(status_code=400, detail="Invalid user parameters")
    except actions.UserNotFound:
...
async def stripe_webhook_handler(
    request: Request, response: Response, db_session=Depends(yield_db_session_from_env),
) -> None:
    """
...
async def add_subscription_handler(
    ...
    try:
        ...
        raise HTTPException(
            status_code=409, detail="Group subscription already attached to group",
        )
    except Exception as e:
        logger.error(repr(e))
