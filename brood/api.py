...
from .rc import yield_rc_async_session
...
async def get_cors():
    async with yield_rc_async_session() as rc:
        ...
        if corse_rc is not None:
            return {"cors": corse_rc}

        await rc.set("cors", ",".join(ORIGINS))
        logger.info("CORS cache updated in Redis")

    return data.CORSResponse(cors=",".join(ORIGINS))


@app.put("/cors", response_model=data.CORSResponse)
async def update_cors(
    cors: List[AnyHttpUrl] = Body(...),
):
    for oc in ORIGINS:
        if oc not in cors:
            raise HTTPException(
                status_code=400,
                detail="Removing default CORS origins is not allowed",
            )

    cors_str = ",".join(cors)
    async with yield_rc_async_session() as rc:
        await rc.set("cors", cors_str)

    return data.CORSResponse(cors=cors_str)
...
async def get_token_types_handler(
    _: Tuple[bool, models.User] = Depends(request_user_authorization),
) -> List[str]:
    """
...
@app.get("/auth", tags=["users"], response_model=data.UserWithGroupsResponse)
async def get_auth_handler(
    user_authorization_with_groups: Tuple[bool, data.UserWithGroupsResponse] = Depends(
        request_user_authorization_with_groups
    )
...
async def get_group_handler(
    ...
    if is_token_restricted:
        raise HTTPException(
            status_code=403,
            detail="Restricted tokens are not authorized to get group.",
        )
...
async def get_group_children_handler(
    ...
    if is_token_restricted:
        raise HTTPException(
            status_code=403,
            detail="Restricted tokens are not authorized to get group children.",
        )
...
async def get_groups_handler(
    ...
    if is_token_restricted:
        raise HTTPException(
            status_code=403,
            detail="Restricted tokens are not authorized to get groups.",
        )
...
async def get_invites_handler(
    ...
    if is_token_restricted:
        raise HTTPException(
            status_code=403,
            detail="Restricted tokens are not authorized to list invites.",
        )
...
async def invite_send_handler(
    ...
    if is_token_restricted:
        raise HTTPException(
            status_code=403,
            detail="Restricted tokens are not authorized to send invites.",
        )
...
async def invite_accept_handler(
    ...
    if is_token_restricted:
        raise HTTPException(
            status_code=403,
            detail="Restricted tokens are not authorized to accept invite.",
        )
...
async def invite_revoke_handler(
    ...
    if is_token_restricted:
        raise HTTPException(
            status_code=403,
            detail="Restricted tokens are not authorized to revoke invites.",
        )
...
async def get_application_handler(
    ...
    if is_token_restricted:
        raise HTTPException(
            status_code=403,
            detail="Restricted tokens are not authorized to create get application.",
        )
    try:
...
async def list_applications_handler(
    ...
    if is_token_restricted:
        raise HTTPException(
            status_code=403,
            detail="Restricted tokens are not authorized to list applications.",
        )
    try:
...
async def delete_application_handler(
    ...
    if is_token_restricted:
        raise HTTPException(
            status_code=403,
            detail="Restricted tokens are not authorized to delete application.",
        )
